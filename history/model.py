...
import csgan.Data_Provider as csdp
...
class DCGAN (object):
	def __init__(self, sess, input_height=108, input_width=108,
		...
		self.dp = csdp.Data_Provider ('../dataset/map1n_allz_rtaapixlw_4096_1b.fits')
	...
	def build_model(self):
		...
		self.REAL_sum = image_summary ("Real", inputs[:3, :, :, :])
		...
		self.d_loss_real = tf.reduce_mean (sigmoid_cross_entropy_with_logits (self.D_logits, tf.ones_like (self.D) * 0.9))
		self.d_loss_fake = tf.reduce_mean (sigmoid_cross_entropy_with_logits (self.D_logits_, tf.ones_like (self.D_) * 0.1))
		self.g_loss = tf.reduce_mean (sigmoid_cross_entropy_with_logits (self.D_logits_, tf.ones_like (self.D_) * 0.9))
	...
	def train(self, config):
		...
		sample = self.dp (self.sample_num, self.output_height)
		print(sample.shape)
		if (self.grayscale):
		...
		for epoch in xrange (config.epoch):
			...
			for idx in xrange (1000):
				batch = self.dp (self.batch_size, self.output_height)
				if self.grayscale:

...
if ~defult_model_build:
    ...
    def discriminator(image, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            h5 = tf.layers.conv2d(inputs=h4, filters=8*dcgan.df_dim, kernel_size=[5,5], 
                     strides=(2,2),padding='same',
                     activation=None, name='d_h5_conv')
            ...
            h6 = tf.layers.dense(inputs=tf.reshape(h5, [dcgan.batch_size, -1]),
                                 units=1,kernel_initializer=tf.random_normal_initializer(stddev=0.02), 
                                 bias_initializer=tf.constant_initializer (0.01),
                                 use_bias=1,activation=None,name='d_h6_lin')
